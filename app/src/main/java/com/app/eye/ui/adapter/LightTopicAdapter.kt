...
import com.app.eye.ui.entity.LightItem
import com.app.eye.ui.entity.LightTag
...
class LightTopicAdapter(data: MutableList<LightItem>) :
    BaseQuickAdapter<LightItem, BaseViewHolder>(data = data, layoutResId =
    R.layout.layout_light_topic_item), LoadMoreModule {
    override fun convert(holder: BaseViewHolder, item: LightItem) {
        ...
        if (!data.tags.isNullOrEmpty()) {
            val tags =
                if (data.tags.size > 2) data.tags.take(2) as MutableList<LightTag> else data.tags
            tags.forEach {
